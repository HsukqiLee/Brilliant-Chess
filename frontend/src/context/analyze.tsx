...
type tabs = 'analyze' | 'selectGame' | 'summary' | 'moves' | 'mistakes'
...
export const AnalyzeContext = createContext<{
    ...
    availableModels: [{ id: string; name: string }[], Dispatch<SetStateAction<{ id: string; name: string }[]>>],
    ...
    customLine: [{ baseMoveNumber: 0, moveNumber: -1, moves: [], arrows: {} }, () => { }],
    variations: [[], () => { }],
...
export default function AnalyzeContextProvider(props: { children: React.ReactNode }) {
    ...
    const [customLine, setCustomLine] = useState<CustomLine>({ baseMoveNumber: 0, moveNumber: -1, moves: [], arrows: {} })
        ...
        async function fetchModels() {
            try {
                ...
                if (res.ok) {
                    ...
                    const isValidSaved = data.models?.some((m: any) => m.id === savedModelId)
    ...
    const gameController: Controller = {
        back: () => {
            if (customLineRef.current.moveNumber > 0) {
                ...
                setCustomLine({ baseMoveNumber: moveNumberRef.current, moveNumber: -1, moves: [], arrows: {} })
    ...
    return (
        <AnalyzeContext.Provider value={{ data: [data, setData], pageState: [pageState, setPageState], game: [game, setGame], players: [players, setPlayers], moveNumber: [moveNumber, setMoveNumber], forward: [forward, setForward], white: [white, setWhite], animation: [animation, setAnimation], playing: [playing, setPlaying], time: [time, setTime], materialAdvantage: [materialAdvantage, setMaterialAdvantage], result: [result, setResult], progress: [progress, setProgress], tab: [tab, setTab], availableModels: [availableModels, setAvailableModels], selectedModelId: [selectedModelId, setSelectedModelId], analyzeController: [analyzeController, setAnalyzeController], customLine: [customLine, setCustomLine], variations: [variations, setVariations], returnedToNormalGame: [returnedToNormalGame, setReturnedToNormalGame], analyzingMove: [analyzingMove, setAnalyzingMove], depth: [depth, setDepth], reviewState: [reviewState, setReviewState], evalWorker: [evalWorker, setEvalWorker], evalWorker2: [evalWorker2, setEvalWorker2], gameController }}>
            {props.children}
