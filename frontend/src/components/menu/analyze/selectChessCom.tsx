...
export const API_BLOCKING_ERROR = [
  "Server might block excesive requests",
  "Slow down your request rate or wait before you try again.",
];
      ...
      <div className="flex-grow flex flex-col justify-center items-center">
        <div className="w-[70%] bg-backgroundBox relative overflow-hidden rounded-borderExtraRoundness text-lg text-foregroundGrey flex flex-col gap-14 pb-4 pt-14 items-center">
          <div className="w-40 flex flex-col items-center gap-4">
            <Files
              className="animate-[pulse_1.25s_cubic-bezier(0.4,_0,_0.6,_1)_infinite;] scale-x-[-1]"
              size={60}
            />
        ...
        <table className="w-full">
          <thead style={{ display: loading ? "none" : "" }}>
            <tr>
              ...
              <th className="py-2 text-left pr-4 notFullDate:pr-8">
                <span className="notFullDate:block hidden">Date</span>
                <span className="notFullDate:hidden block">Day</span>
              </th>
            </tr>
          </thead>
          <tbody>
            {gamesInfo.map((gameInfo, i) => {
              ...
              return (
                <tr
                  title={`Time Class: ${capitalizeFirst(timeClass)}`}
                  onClick={() => setData({ format: "pgn", string: pgn })}
                  className="border-b-[1px] cursor-pointer select-none border-border transition-colors hover:bg-backgroundBoxHover"
                  key={i}
                >
                  <td className="text-base flex flex-col py-4 w-60 overflow-hidden pl-8">
                    <div className="flex flex-row items-center gap-2">
                      <div
                        className={`h-4 min-h-4 w-4 min-w-4 bg-evaluationBarWhite rounded-borderRoundness ${whiteWon ? "border-[3px] border-winGreen" : ""}`}
                      />
                      {whiteName} ({whiteElo})
                    </div>
                    <div className="flex flex-row items-center gap-2">
                      <div
                        className={`h-4 w-4 bg-evaluationBarBlack rounded-borderRoundness ${blackWon ? "border-[3px] border-winGreen" : ""}`}
                      />
                      {blackName} ({blackElo})
                    </div>
                  </td>
                  <td className="py-2 pl-3 pr-4">
                    <div className="flex flex-row items-center gap-2">
                      <div className="flex w-4 flex-col text-foregroundGrey font-bold text-base">
                        <span>{whiteWon ? 1 : blackWon ? 0 : <>&#189;</>}</span>
                        <span>{blackWon ? 1 : whiteWon ? 0 : <>&#189;</>}</span>
                      </div>
                      <div
                        style={{ mixBlendMode: "screen" }}
                        className={`h-4 w-4 rounded-borderRoundness text-lg font-extrabold flex justify-center items-center text-black ${isWin ? "bg-winGreen" : isLoss ? "bg-lossRed" : "bg-foregroundGrey"}`}
                      >
                        <div className="w-fit h-fit ml-px">
                          {isWin ? "+" : isLoss ? "-" : "="}
                        </div>
                      </div>
                    </div>
                  ...
                  <td className="py-4 pr-4 notFullDate:pr-8 text-nowrap text-sm">
                    <div className="flex flex-row items-baseline">
                      <span className="hidden notFullDate:block">
...
export default function SelectChessComGame(props: {
  username: string;
  depth: number;
  stopSelecting: () => void;
}) {
  ...
  const [dates, setDates] = useState<
    { month: string; year: string; url: string }[]
  >([]);
  ...
  return (
    <div
      className={`overflow-x-hidden overflow-y-auto ${loading ? "flex flex-col justify-center flex-grow" : ""}`}
    >
      <h1
        style={{ display: loading ? "none" : "" }}
        className="text-2xl py-4 px-8 sticky text-foreground"
      >
        <a
          target="_blank"
          href={`${PLAYER_URL}${username}`}
          className="hover:underline text-backgroundBoxBoxHighlightedHover text-3xl font-bold"
        >
          {username}
        </a>
        &apos;s games
      ...
      <div className="flex flex-col w-full">
        {loading ? (
          <Loading whatIsLoading="Archives" abort={stopSelecting} />
        ) : null}
        {dates.map((date, i) => {
          return (
            <div className="w-full" key={i}>
              <button
                ...
                type="button"
                className={`${hovered === i || selected === i ? "text-foregroundHighlighted" : "text-foregroundGrey"} hover:bg-backgroundBoxHover w-full tracking-wide transition-colors text-2xl px-8 py-4 flex flex-row justify-between items-center`}
              >
                <span>
                  <b>{date.year}</b> {getMonthName(Number(date.month))}
                </span>
                <div
                  style={{
                    opacity: hovered === i || selected === i ? "100" : "0",
                    transform: `rotate(${selected !== i ? "180deg" : "0"})`,
                  }}
                  className="transition-opacity"
                >
                  <Arrow class="fill-foregroundHighlighted" />
                </div>
              </button>
              {selected === i ? (
                <Games
