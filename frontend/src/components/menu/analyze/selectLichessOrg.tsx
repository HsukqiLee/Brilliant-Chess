  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const newGamesInfo: typeof gamesInfo = jsonArr
          .map((json) => {
        ...
        await pushPageWarning(
          setErrors,
          API_BLOCKING_ERROR[0],
          API_BLOCKING_ERROR[1],
        );
      }
    })();
  }, []);
...
export default function SelectLichessOrgGame(props: {
  username: string;
  depth: number;
  stopSelecting: () => void;
}) {
  ...
  const [dates, setDates] = useState<
    { month: number; year: number; url: string }[]
  >([]);
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const resFirstGame = await fetch(
          `https://lichess.org/api/games/user/${username}?sort=dateAsc&max=1`,
          { headers: { Accept: "application/x-ndjson" } },
        );
        ...
        const resLastGame = await fetch(
          `https://lichess.org/api/games/user/${username}?sort=dateDesc&max=1`,
          { headers: { Accept: "application/x-ndjson" } },
        );
        ...
        while (currentDate <= dateLastGame) {
          ...
          newDates.push({
            month: month + 1,
            year,
            url: `https://lichess.org/api/games/user/${username}?since=${sinceDate.getTime()}&until=${untilDate.getTime()}&pgnInJson=true`,
          });
    ...
    >
      ...
      >
        <a
          ...
          className="hover:underline text-foregroundHighlighted text-3xl font-bold"
        >
          {username}
        </a>
        &apos;s games
        ...
        {dates.map((date, i) => {
          return (
            <div key={i}>
              ...
              >
                <span>
                  <b>{date.year}</b> {getMonthName(date.month)}
                </span>
                <div
                  style={{
