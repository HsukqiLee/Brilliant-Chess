...
function computeAverageCpl(moves: move[], color: Color): number {
  const losses: number[] = [];
  ...
  return losses.length ? avg(losses) : 0;
}

function isEndOpening(position: position) {
  let nonDeveloppedBlack = 0,
    nonDeveloppedWhite = 0;
  for (const square of position[0]) {
    if (square?.color === "w") continue;
    if (square?.type === "n" || square?.type === "b" || square?.type === "q")
      nonDeveloppedBlack++;
  }
...
function pushAccuracy(
  arr: { w: number[]; b: number[] },
  value: number,
  color: Color,
) {
...
export default function PlayersAccuracy(props: {
  reducedSummary: boolean;
  players: players;
  moves: move[];
  accuracy: [
    { w: number; b: number },
    Dispatch<SetStateAction<{ w: number; b: number }>>,
  ];
  setAccuracyPhases: Dispatch<SetStateAction<accuracyPhases>>;
}) {
  const { players, setAccuracyPhases, moves } = props;
  const [accuracy, setAccuracy] = props.accuracy;

  const averageCplWhite = computeAverageCpl(moves, "w");
  const averageCplBlack = computeAverageCpl(moves, "b");

  useEffect(() => {
    const accuracies: { w: number[]; b: number[] } = { w: [], b: [] };
    const accuraciesPhases: accuracyPhases = {
      opening: { w: [], b: [] },
      middlegame: { w: [], b: [] },
      endgame: { w: [], b: [] },
    };
  ...
  return (
    <div className="w-[85%] flex flex-col items-end gap-3">
      <div className="reduceSummary:w-[262px] w-[160px] flex flex-row justify-between font-extrabold">
        <div className="reduceSummary:w-20 w-16 flex flex-row justify-center whitespace-nowrap overflow-visible">
      ...
      <div className="flex flex-row w-full justify-between items-center">
        <span className="font-bold text-foregroundGrey reduceSummary:text-lg text-base">
          Players
        </span>
        <div className="flex flex-row reduceSummary:w-[262px] w-[160px] justify-between">
          <div className="reduceSummary:h-20 reduceSummary:w-20 h-16 w-16 flex flex-row justify-center items-end bg-backgroundProfileWhite rounded-borderRoundness">
            <Profile
              width={props.reducedSummary ? 58 : 70}
              height={props.reducedSummary ? 58 : 70}
              class="fill-foregroundProfileWhite"
            />
          </div>
          <div className="reduceSummary:h-20 reduceSummary:w-20 h-16 w-16 flex flex-row justify-center items-end bg-backgroundProfileBlack rounded-borderRoundness">
            <Profile
              width={props.reducedSummary ? 58 : 70}
              height={props.reducedSummary ? 58 : 70}
              class="fill-foregroundProfileBlack"
            />
          </div>
        </div>
      </div>
      <div className="flex flex-row w-full justify-between items-center">
        ...
        <div className="flex flex-row reduceSummary:w-[262px] w-[160px] justify-between">
          <RatingBox
            fontSize={props.reducedSummary ? 18 : undefined}
            width={props.reducedSummary ? 64 : undefined}
            paddingY={props.reducedSummary ? 4 : undefined}
            white
          >
            {accuracy.w.toFixed(1)}
          </RatingBox>
          <RatingBox
            fontSize={props.reducedSummary ? 18 : undefined}
            width={props.reducedSummary ? 64 : undefined}
