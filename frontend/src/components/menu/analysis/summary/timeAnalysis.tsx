...
export default function TimeAnalysis(props: { moves: move[] }) {
    ...
    const [chartMode, setChartMode] = useState<"remaining" | "spent">("remaining")
    const [hoveredPoint, setHoveredPoint] = useState<{ white?: HistoryPoint; black?: HistoryPoint; ply: number } | null>(null)
    ...
    const movesWithClk = moves.filter(m => m.clk !== undefined && m.clk !== null)
    ...
    const firstWhiteMove = moves.find(m => m.color === "b" && m.clk !== undefined && m.clk !== null) // White's move has color 'b' (next turn is Black)
    const firstBlackMove = moves.find(m => m.color === "w" && m.clk !== undefined && m.clk !== null) // Black's move has color 'w'
    ...
    const whiteAvg = whiteSpentTimes.length ? whiteSpentTimes.reduce((a, b) => a + b, 0) / whiteSpentTimes.length : 0
    const blackAvg = blackSpentTimes.length ? blackSpentTimes.reduce((a, b) => a + b, 0) / blackSpentTimes.length : 0
    ...
    const maxClkVal = Math.max(baseTime, ...whiteHistory.map(h => h.clk), ...blackHistory.map(h => h.clk))
    const minClkVal = Math.min(...whiteHistory.map(h => h.clk), ...blackHistory.map(h => h.clk), 0)
    ...
    const whitePath = chartMode === "remaining" ? buildRemainingPath(whiteHistory) : buildSpentPath(whiteHistory)
    const blackPath = chartMode === "remaining" ? buildRemainingPath(blackHistory) : buildSpentPath(blackHistory)
    ...
    const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
        ...
        whiteHistory.forEach(h => {
            if (!closestWhite || Math.abs(h.ply - targetPly) < Math.abs(closestWhite.ply - targetPly)) {
                closestWhite = h
            }
        })

        blackHistory.forEach(h => {
            if (!closestBlack || Math.abs(h.ply - targetPly) < Math.abs(closestBlack.ply - targetPly)) {
                closestBlack = h
            }
        })

        const activePly = closestWhite && closestBlack 
            ? (Math.abs(closestWhite.ply - targetPly) < Math.abs(closestBlack.ply - targetPly) ? closestWhite.ply : closestBlack.ply)
    ...
    return (
        <div ref={containerRef} className="w-[85%] flex flex-col gap-3 bg-backgroundBoxDarker rounded-borderRoundness px-3 py-2 border border-neutral-750">
            ...
            <div className="relative select-none">
                ...
                >
                        ...
                        yGridLines().map(clk => {
                            ...
                            return (
                                <g key={clk} className="opacity-10">
                                    ...
                                    <text x={paddingX - 4} y={y + 3} fill="white" fontSize={7} textAnchor="end">{Math.floor(clk / 60)}m</text>
                                </g>
                            )
                        })
                    ) : (
                        [0, maxSpentVal / 2, maxSpentVal].map((spent, idx) => {
                            ...
                            return (
                                <g key={idx} className="opacity-10">
                                    <line x1={paddingX} y1={y} x2={width - paddingX} y2={y} stroke="white" strokeWidth={1} strokeDasharray="3 3" />
                                    <text x={paddingX - 4} y={y + 3} fill="white" fontSize={7} textAnchor="end">{spent.toFixed(0)}s</text>
                                </g>
                            )
                        })
                    )}
                    ...
                    <line x1={width - paddingX} y1={paddingY} x2={width - paddingX} y2={height - paddingY} stroke="rgba(255,255,255,0.1)" strokeWidth={1} />

                    {/* Chart lines */}
                    {whitePath && (
                        <path d={whitePath} fill="none" stroke="var(--foregroundProfileWhite)" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
                    )}
                    {blackPath && (
                        <path d={blackPath} fill="none" stroke="#22c55e" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />
                    )}
                        ...
                        <>
                            ...
                            {hoveredPoint.white && (
                                <circle
                                    ...
                                    cy={chartMode === "remaining" ? getYRemaining(hoveredPoint.white.clk) : getYSpent(hoveredPoint.white.spent)}
                                    r={4}
                            ...
                            {hoveredPoint.black && (
                                <circle
                                    ...
                                    cy={chartMode === "remaining" ? getYRemaining(hoveredPoint.black.clk) : getYSpent(hoveredPoint.black.spent)}
                                    r={4}
                                    fill="#22c55e"
                                    stroke="black"
                                    strokeWidth={1}
                        ...
                        {hoveredPoint.white && (
                            <div className="flex gap-2 items-center justify-between text-neutral-300">
                                ...
                                <span className="font-mono">
                                    {chartMode === "remaining" ? formatDuration(hoveredPoint.white.clk) : `+${hoveredPoint.white.spent.toFixed(1)}s`}
                                </span>
                            </div>
                        )}
                        {hoveredPoint.black && (
                            <div className="flex gap-2 items-center justify-between text-neutral-300">
                                ...
                                <span className="font-mono text-green-400">
                                    {chartMode === "remaining" ? formatDuration(hoveredPoint.black.clk) : `+${hoveredPoint.black.spent.toFixed(1)}s`}
                                </span>
                            </div>
                        )}
                    </div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-1 border-t border-neutral-750 pt-2.5">
                ...
                <div className="flex flex-col items-center text-center p-1 bg-backgroundBoxBox/20 rounded-borderRoundness border border-neutral-800/40">
                    <span className="text-[8px] text-foregroundGrey uppercase tracking-wider font-extrabold">Avg Move</span>
                ...
                {/* Peak Thought Time */}
                <div className="flex flex-col items-center text-center p-1 bg-backgroundBoxBox/20 rounded-borderRoundness border border-neutral-800/40">
                    <span className="text-[8px] text-foregroundGrey uppercase tracking-wider font-extrabold">Peak Think</span>
                    <div className="flex flex-col gap-0.5 mt-1 text-[9px] font-bold">
                        <span className="text-neutral-200 truncate max-w-[70px]">
                            ⚪ {whitePeak ? `${(whitePeak as HistoryPoint).spent.toFixed(0)}s` : "-"}
                        </span>
                        <span className="text-green-400 truncate max-w-[70px]">
                            🟢 {blackPeak ? `${(blackPeak as HistoryPoint).spent.toFixed(0)}s` : "-"}
                        </span>
                    </div>
                </div>
                ...
                {/* Time Trouble Moves */}
                <div className="flex flex-col items-center text-center p-1 bg-backgroundBoxBox/20 rounded-borderRoundness border border-neutral-800/40">
                    <span className="text-[8px] text-foregroundGrey uppercase tracking-wider font-extrabold">Time Trouble</span>
                    <div className="flex flex-col gap-0.5 mt-1 font-mono text-[10px] font-bold">
