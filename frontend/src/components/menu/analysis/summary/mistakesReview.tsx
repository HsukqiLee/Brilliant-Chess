...
export default function MistakesReview() {
    ...
    const getMistakes = (color: "w" | "b") => {
        ...
        for (let i = 1; i < game.length; i++) {
            ...
            if (playerColor === color && ["blunder", "mistake", "miss"].includes(move.moveRating ?? "")) {
                list.push(i)
            }
        }
        return list
        ...
        return (
            <div className="flex flex-col gap-6 p-4 items-center justify-center text-center">
                <div className="w-[80px] h-[80px] flex justify-center items-center rounded-full bg-backgroundBoxBox select-none">
                    <Image draggable={false} alt="review" src={`${process.env.NEXT_PUBLIC_BASE_PATH}/images/type.svg`} width={45} height={45} />
                </div>
                <div>
                    <h2 className="text-2xl font-extrabold text-foreground mb-2">Mistakes Review</h2>
                    <p className="text-foregroundGrey text-sm max-w-[320px]">
                        Review the critical blunders, mistakes, or missed tactical ideas in your game and find the optimal moves.
                    </p>
                </div>
                ...
                <div className="w-full flex flex-col gap-3 max-w-[360px]">
                    ...
                    >
                        ...
                        <span className="bg-black/25 px-3 py-1 rounded-full text-sm">{whiteMistakes.length} mistakes</span>
                    </button>
                    ...
                    >
                        ...
                        <span className="bg-black/25 px-3 py-1 rounded-full text-sm">{blackMistakes.length} mistakes</span>
                    </button>
                </div>
            </div>
        )
    ...
    const uppercaseRating = ratingName.charAt(0).toUpperCase() + ratingName.slice(1)
        ...
        <div className="flex flex-col gap-4 p-4 h-full">
            <div className="flex flex-row justify-between items-center bg-backgroundBoxBox p-3 rounded-borderRoundness border border-border">
                <div className="flex flex-col">
                    <span className="text-foregroundGrey text-xs font-bold uppercase tracking-wider">Mistakes Review</span>
            ...
            {reviewState.solved && (
                <div className="flex flex-col gap-3 bg-green-950/30 text-green-300 border border-green-800/40 p-4 rounded-borderRoundness animate-fade-in">
                    ...
                    >
                        {currentReviewIndex + 1 < mistakesList.length ? "Next Mistake" : "Finish Review"}
                    </button>
                </div>
            )}
            ...
            {reviewState.wrong && (
                <div className="flex flex-col gap-3 bg-red-950/20 text-red-300 border border-red-900/30 p-4 rounded-borderRoundness animate-shake">
                    <div className="font-extrabold text-md">That is not the best move! ❌</div>
                    <div className="text-sm text-red-300/80">Try to look for a better tactical response or position improvement.</div>
                    <div className="flex flex-row gap-2 mt-1">
                        <button
                            onClick={() => setReviewState(prev => ({ ...prev, wrong: false, playedMove: null }))}
