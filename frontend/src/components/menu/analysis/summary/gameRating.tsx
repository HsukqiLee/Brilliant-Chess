...
function getRating(accuracy: number): number {
  ...
  const normalizedAccuracy =
    (accuracy - minAccuracy) / (maxAccuracy - minAccuracy);
...
function RatingIcon(props: {
  ratingPhase: { rating: moveRating | null; accuracy: string | null };
  titleText: string;
}) {
  ...
  if (!rating || !accuracy)
    return (
      <span
        className="text-xl"
        title="Not enough moves to evaluate this phase."
      >
        -
      </span>
    );
    ...
    <div className="w-[85%] flex flex-col items-end gap-3">
      <div className="flex flex-row w-full justify-between items-center">
        <span className="font-bold text-foregroundGrey reduceSummary:text-lg text-base">
          {reducedSummary ? "" : "Game "}Rating
        </span>
        <div className="flex flex-row reduceSummary:w-[262px] reduceSummary:min-w-[262px] min-w-[160px] w-[160px] justify-between">
          <RatingBox
            fontSize={reducedSummary ? 18 : undefined}
            width={reducedSummary ? 64 : undefined}
            paddingY={reducedSummary ? 4 : undefined}
            white
          >
            {topElo(getRating(accuracy.w))}
          </RatingBox>
          <RatingBox
            fontSize={reducedSummary ? 18 : undefined}
            width={reducedSummary ? 64 : undefined}
      ...
      <div className="flex flex-row w-full justify-between items-center">
        ...
        <div className="flex flex-row reduceSummary:w-[262px] w-[160px] justify-between">
          <div className="reduceSummary:w-20 w-16 flex items-center justify-center select-none">
            <RatingIcon
              ratingPhase={getRatingPhase(accuracyPhases.opening.w)}
              titleText="Opening Accuracy"
            />
          </div>
          <div className="reduceSummary:w-20 w-16 flex items-center justify-center select-none">
            <RatingIcon
              ratingPhase={getRatingPhase(accuracyPhases.opening.b)}
              titleText="Opening Accuracy"
            />
          </div>
        </div>
      ...
      <div className="flex flex-row w-full justify-between items-center">
        ...
        <div className="flex flex-row reduceSummary:w-[262px] w-[160px] justify-between">
          <div className="reduceSummary:w-20 w-16 flex items-center justify-center select-none">
            <RatingIcon
              ratingPhase={getRatingPhase(accuracyPhases.middlegame.w)}
              titleText="Middlegame Accuracy"
            />
          </div>
          <div className="reduceSummary:w-20 w-16 flex items-center justify-center select-none">
            <RatingIcon
              ratingPhase={getRatingPhase(accuracyPhases.middlegame.b)}
              titleText="Middlegame Accuracy"
            />
          </div>
        </div>
      ...
      <div className="flex flex-row w-full justify-between items-center">
        ...
        <div className="flex flex-row reduceSummary:w-[262px] w-[160px] justify-between">
          <div className="reduceSummary:w-20 w-16 flex items-center justify-center select-none">
            <RatingIcon
              ratingPhase={getRatingPhase(accuracyPhases.endgame.w)}
              titleText="Endgame Accuracy"
            />
          </div>
          <div className="reduceSummary:w-20 w-16 flex items-center justify-center select-none">
            <RatingIcon
              ratingPhase={getRatingPhase(accuracyPhases.endgame.b)}
              titleText="Endgame Accuracy"
            />
          </div>
        </div>
