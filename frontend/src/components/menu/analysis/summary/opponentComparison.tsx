...
function computeAverageCpl(moves: move[], color: "w" | "b"): number {
  ...
  return losses.length
    ? losses.reduce((acc, cur) => acc + cur, 0) / losses.length
    : 0;
}

export default function OpponentComparison(props: {
  moves: move[];
  players: players;
  ...
  const userAccuracy =
    selectedColor === "w" ? accuracy.w || 0 : accuracy.b || 0;
  ...
  const targetData =
    RATING_LEVELS.find((l) => l.elo === comparedElo) || RATING_LEVELS[2];
  ...
  return (
    <div className="w-[85%] flex flex-col gap-4 bg-backgroundBoxBox p-4 rounded-borderRoundness border border-border">
      ...
      <div className="flex flex-col gap-1.5">
        <span className="text-xs font-bold text-foregroundGrey">
          对比目标分段:
        </span>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1 p-2 bg-neutral-950/40 hover:bg-neutral-950/70 rounded border border-neutral-900 hover:border-neutral-800/80 transition-all duration-200">
          ...
          <div className="flex flex-row justify-between text-xs text-foregroundGrey mt-1">
            ...
            <span>
              同级平均: <strong>{targetData.accuracy.toFixed(1)}%</strong>
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 p-2 bg-neutral-950/40 hover:bg-neutral-950/70 rounded border border-neutral-900 hover:border-neutral-800/80 transition-all duration-200">
          ...
          <div className="flex flex-row justify-between text-xs text-foregroundGrey mt-1">
            <span>
              你的表现:{" "}
              <strong className="text-foreground">{userCpl.toFixed(1)}</strong>
            </span>
            <span>
              同级平均: <strong>{targetData.cpl.toFixed(1)}</strong>
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 p-2 bg-neutral-950/40 hover:bg-neutral-950/70 rounded border border-neutral-900 hover:border-neutral-800/80 transition-all duration-200">
          ...
          <div className="flex flex-row justify-between text-xs text-foregroundGrey mt-1">
            <span>
              你的表现:{" "}
              <strong className="text-foreground">{userCounts.blunders}</strong>
            </span>
            <span>
              同级平均: <strong>{targetData.blunders.toFixed(1)}</strong>
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1 p-2 bg-neutral-950/40 hover:bg-neutral-950/70 rounded border border-neutral-900 hover:border-neutral-800/80 transition-all duration-200">
          ...
          <div className="flex flex-row justify-between text-xs text-foregroundGrey mt-1">
            <span>
              你的表现:{" "}
              <strong className="text-foreground">{userCounts.mistakes}</strong>
            </span>
            <span>
              同级平均: <strong>{targetData.mistakes.toFixed(1)}</strong>
            </span>
          </div>
        </div>
      </div>
