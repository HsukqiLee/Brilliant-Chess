...
export default function getOverallGameComment(
  ...
  const getComment = (commentList: string[], winner: string, loser: string) => {
    ...
    return commentList[Math.floor(Math.random() * commentList.length)]
      .replace(/__WINNER__/g, tab[0] + winner + tab[1])
      .replace(/__LOSER__/g, tab[0] + loser + tab[1]);
  };
  ...
  switch (result) {
    ...
    case "1/2-1/2":
      ...
      comment = getComment(
        COMMENTS.draw,
        randomBool ? white : black,
        randomBool ? black : white,
      );
      break;
    case "*":
    case "":
