...
export default function OpeningExplorer(props: { fen?: string }) {
    ...
    const [selectedSpeeds, setSelectedSpeeds] = useState<string[]>(["bullet", "blitz", "rapid", "classical"])
    const [selectedRatings, setSelectedRatings] = useState<number[]>([1600, 1800, 2000, 2200, 2500])
    ...
    function toggleSpeed(speed: string) {
        setSelectedSpeeds(prev => {
            const next = prev.includes(speed) ? prev.filter(s => s !== speed) : [...prev, speed]
    ...
    function toggleRating(rating: number) {
        setSelectedRatings(prev => {
            const next = prev.includes(rating) ? prev.filter(r => r !== rating) : [...prev, rating]
            return next.length > 0 ? next : prev
        })
    }
    ...
    const cacheKey = useMemo(() => {
        ...
        const speedStr = source === "lichess" ? selectedSpeeds.slice().sort().join(",") : ""
        const ratingStr = source === "lichess" ? selectedRatings.slice().sort().join(",") : ""
    ...
    }, [fen, cacheKey, source, pat, isTokenLoaded, selectedSpeeds, selectedRatings])

    const totals = data ? data.white + data.black + data.draws : 0
    const topMoves = data?.moves
        ? [...data.moves].sort((a, b) => (b.white + b.black + b.draws) - (a.white + a.black + a.draws)).slice(0, 8)
        : []
            ...
            <div className="flex flex-row items-center justify-between">
                ...
                {!isUnauthorized && (
                    <div className="flex flex-row gap-2">
                        <button type="button" onClick={() => setSource("lichess")} className={`text-xs font-bold px-2 py-1 rounded-borderRoundness transition-colors ${source === "lichess" ? "bg-backgroundBoxBox text-foreground" : "text-foregroundGrey hover:bg-backgroundBoxBox"}`}>Lichess</button>
                        <button type="button" onClick={() => setSource("masters")} className={`text-xs font-bold px-2 py-1 rounded-borderRoundness transition-colors ${source === "masters" ? "bg-backgroundBoxBox text-foreground" : "text-foregroundGrey hover:bg-backgroundBoxBox"}`}>Masters</button>
                    </div>
                )}
            </div>
            ...
            {showSettings && pat && (
                <div className="flex flex-col gap-1.5 p-2 bg-backgroundBoxBox/40 rounded-borderRoundness border border-neutral-750 text-xs">
                    <div className="flex justify-between items-center">
                        <span className="font-bold text-foregroundGrey">Lichess API Token:</span>
                        <button 
                            type="button" 
            ...
            {isUnauthorized ? (
                <div className="flex flex-col gap-2.5 py-1 text-xs">
                    <p className="text-foregroundGrey leading-relaxed text-[11px]">
                        Lichess now requires a Personal Access Token (PAT) to view Opening Explorer stats. It is free and quick to create.
                    </p>
                    <a 
                ...
                <>
                    ...
                    {!fen ? (
                        <div className="text-xs text-foregroundGrey mt-1">No position loaded.</div>
                    ) : null}
                    {loading ? <div className="text-xs text-foregroundGrey mt-1">Loading opening stats...</div> : null}
                    {error && error !== "unauthorized" ? <div className="text-xs text-highlightMistake mt-1">{error}</div> : null}
                            ...
                            {topMoves.length ? (
                                <ul className="flex flex-col gap-1">
                                    {topMoves.map((move) => {
                                        ...
                                        const width = totals ? Math.round((total / totals) * 100) : 0

                                        return (
                                            <li key={move.uci} className="flex flex-col gap-1">
                                                <div className="flex flex-row justify-between text-xs">
                                                    <span className="font-extrabold text-foreground">{move.san}</span>
                                                    <span className="text-foregroundGrey">{total}</span>
                                                </div>
                                                <div className="h-2 rounded-borderRoundness bg-backgroundBoxBox">
                                                    <div className="h-2 rounded-borderRoundness bg-highlightGreat" style={{ width: `${width}%` }} />
                                                </div>
                                                <div className="flex flex-row justify-between text-[10px] text-foregroundGrey">
                                                    <span>W {Math.round((move.white / total) * 100)}%</span>
                                                    <span>D {Math.round((move.draws / total) * 100)}%</span>
                                                    <span>B {Math.round((move.black / total) * 100)}%</span>
                                                </div>
                                            </li>
                                        )
                                    })}
                                ...
                                <div className="text-xs text-foregroundGrey">No opening moves found.</div>
                            )}

                            {/* Top Games List */}
                            {data.topGames && data.topGames.length > 0 && (
                                <div className="flex flex-col gap-1.5 mt-2 border-t border-neutral-700/40 pt-2">
                                    <div className="text-[10px] font-bold text-foregroundGrey uppercase tracking-wider">Top Games</div>
                                    <ul className="flex flex-col gap-1">
                                        {data.topGames.slice(0, 4).map((game) => {
                                            const resultLabel = game.winner === "white" ? "1-0" : game.winner === "black" ? "0-1" : "½-½"
                                            const resultColor = game.winner === "white" 
                                                ...
                                                <li key={game.id}>
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex flex-row items-center gap-1.5 shrink-0">
                                                            <span className="text-[10px] text-foregroundGrey opacity-80">{game.year}</span>
                                                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-borderRoundness ${resultColor}`}>
                                                                {resultLabel}
                                                            </span>
                                                        </div>
                                                    </a>
