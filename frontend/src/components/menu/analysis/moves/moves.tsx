...
export default function Moves(props: { moves: move[], mainlineMoves?: move[], mainlineMoveNumber?: number, setMainlineMoveNumber?: (moveNumber: number) => void, overallGameComment: string, container: HTMLElement, moveNumber: number, setMoveNumber: (moveNumber: number) => void, setAnimation: (animation: boolean) => void, setForward: (forward: boolean) => void, customLine: CustomLine, returnedToNormalGame: square[] | null, analyzingMove: boolean }) {
    const { moves, mainlineMoves, mainlineMoveNumber, setMainlineMoveNumber, overallGameComment, container, moveNumber, setMoveNumber, setAnimation, setForward, customLine, returnedToNormalGame, analyzingMove } = props
    ...
    const variationSeries = hasVariation && mainlineMoves
        ? { baseMoveNumber: customLine.baseMoveNumber, moves: customLine.moves, activeMoveNumber: customLine.moveNumber }
        : undefined
        ...
        <div ref={componentRef} className="flex flex-col gap-3 items-center h-full">
            <div ref={commentsRef} className="w-full flex flex-col items-center">
                <Comments 
                    ...
                    evaluation={analyzingMove ? previousMove?.previousStaticEvals?.[0] ?? ["cp", "0"] : move?.previousStaticEvals?.[0] ?? ["cp", "0"]} 
                    white={analyzingMove ? previousMove?.color === WHITE : move?.color === WHITE} 
                    overallGameComment={overallGameComment} 
                    fen={analyzingMove ? previousMove?.fen : move?.fen}
                    bestMove={analyzingMove ? previousMove?.bestMoveSan : move?.bestMoveSan}
                    evalBefore={analyzingMove ? previousMove?.previousStaticEvals?.[1] : move?.previousStaticEvals?.[1]}
                    evalAfter={analyzingMove ? previousMove?.previousStaticEvals?.[0] : move?.previousStaticEvals?.[0]}
                />
            </div>
            ...
            <div ref={gameChartRef}>
                <GameChart container={container} moves={chartMoves} moveNumber={chartMoveNumber} setMoveNumber={handleChartMoveSelect} setAnimation={setAnimation} setForward={setForward} variation={variationSeries} />
            </div>
        </div>
    )
}