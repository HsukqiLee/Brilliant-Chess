...
type VariationSeries = {
    baseMoveNumber: number
    moves: move[]
    activeMoveNumber: number
}

export default function GameChart(props: { moves: move[], container: HTMLElement, moveNumber: number, setMoveNumber: (moveNumber: number) => void, setAnimation: (animation: boolean) => void, setForward: (forward: boolean) => void, variation?: VariationSeries }) {
    const { moves, container, moveNumber, setMoveNumber, setAnimation, setForward, variation } = props
    ...
    const totalMoves = Math.max(moves.length - 1, 1)
    ...
    function getChartPoint(move: move, ply: number) {
        const clampedPly = Math.min(Math.max(ply, 0), totalMoves)
        const xRelation = getMoveX(clampedPly, totalMoves)
        const yRelation = 1 - getMoveY(move, clampedPly)

        return {
            x: size.width * xRelation,
            y: size.height * yRelation,
        }
    }
    ...
    const variationPoints = (() => {
        ...
        const baseMove = moves[variation.baseMoveNumber]

        if (baseMove) {
            points.push(getChartPoint(baseMove, variation.baseMoveNumber))
        }

        variation.moves.forEach((move, index) => {
            const ply = variation.baseMoveNumber + index + 1
            points.push(getChartPoint(move, ply))
        })

        return points
    })()

    const variationPath = variationPoints.length
        ? `M ${variationPoints[0].x} ${variationPoints[0].y} ${variationPoints.slice(1).map(point => `L ${point.x} ${point.y}`).join(' ')}`
        : ''

    const activeVariationMove = variation && variation.activeMoveNumber >= 0
        ? variation.moves[variation.activeMoveNumber]
        : undefined

    const activeVariationPoint = activeVariationMove && variation
        ? getChartPoint(activeVariationMove, variation.baseMoveNumber + variation.activeMoveNumber + 1)
        : null
    ...
    return (
        <svg onClick={changeMoveNumber} onMouseMove={hoverMove} onMouseLeave={() => setHoveredMove(NaN)} width={size.width} height={size.height} className="bg-evaluationBarBlack rounded-borderRoundness">
            ...
            {variationPath ? (
                <path d={variationPath} fill="none" stroke="var(--highlightGreat)" strokeWidth={2} strokeDasharray="6 4" />
            ) : null}
            {activeVariationPoint ? (
                <circle cx={activeVariationPoint.x} cy={activeVariationPoint.y} r={5} fill="var(--highlightGreat)" />
            ) : null}
