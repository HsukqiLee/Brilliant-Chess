...
export default function Name(props: {
  ...
  return (
    <div className="flex flex-row items-start vertical:text-sm text-[10px] font-bold text-foregroundHighlighted vertical:gap-2 gap-[6px] flex-grow overflow-x-auto overflow-y-hidden">
      <div
        className={`vertical:h-11 vertical:w-11 h-7 w-7 flex flex-row justify-center items-end ${white ? "bg-backgroundProfileWhite" : "bg-backgroundProfileBlack"}`}
      >
        <Profile
          width={profileSize}
          height={profileSize}
          class={`${white ? "fill-foregroundProfileWhite" : "fill-foregroundProfileBlack"}`}
        />
      </div>
