...
function getCastleRookFromSquare(
  castle: "k" | "q" | undefined,
  whiteMoving: boolean,
): square | undefined {
  if (!castle) return;
...
function getCastleRookToSquare(
  castle: "k" | "q" | undefined,
  whiteMoving: boolean,
): square | undefined {
  if (!castle) return;
...
export function Arrow(props: {
  ...
  if (!move[1]) {
    ...
    let rounded: string = "";
    if (move[0].row === 0 && move[0].col === 0)
      rounded = white
        ? "rounded-tl-borderRoundness"
        : "rounded-br-borderRoundness";
    if (move[0].row === 7 && move[0].col === 0)
      rounded = white
        ? "rounded-bl-borderRoundness"
        : "rounded-tr-borderRoundness";
    if (move[0].row === 0 && move[0].col === 7)
      rounded = white
        ? "rounded-tr-borderRoundness"
        : "rounded-bl-borderRoundness";
    if (move[0].row === 7 && move[0].col === 7)
      rounded = white
        ? "rounded-br-borderRoundness"
        : "rounded-tl-borderRoundness";
    ...
    return (
      <div
        style={{
          top: white ? elementPosition.y : "",
          bottom: !white ? elementPosition.y : "",
          left: white ? elementPosition.x : "",
          right: !white ? elementPosition.x : "",
          width: size,
          height: size,
        }}
        className={`absolute opacity-80 z-[10] ${props.class} ${rounded}`}
      />
    );
  }
    ...
    return (
      <svg
        style={{
          top: positionY,
          left: positionX,
          transformOrigin: `${arrowHeadWidth / 2}px 0`,
          transform: getTransform(),
        }}
        className={`absolute opacity-65 z-[70] pointer-events-none ${props.class}`}
        width={width}
        height={height}
      ...
      >
        <polygon
          strokeWidth={0}
          points={`0,${arrowHeadHeight} ${shortLineCenter},0 ${arrowHeadWidth},${arrowHeadHeight}`}
        />
        <path
          fill="none"
          strokeWidth={lineWidth}
          d={`M ${shortLineCenter} ${arrowHeadHeight - 1} L ${shortLineCenter} ${longLineCenter} L ${width} ${longLineCenter}`}
        />
      </svg>
    );
  }
  ...
  return (
    <svg
      style={{
        top: white ? positionY : "",
        bottom: !white ? positionY : "",
        left: white ? positionX : "",
        right: !white ? positionX : "",
    ...
    >
      <line
        ...
        y1={height}
        x2={lineCenter}
        y2={arrowHeadHeight - 1}
        strokeWidth={lineWidth}
        markerEnd="url(#arrowhead)"
      />
      <polygon
        strokeWidth={0}
        points={`0,${arrowHeadHeight} ${lineCenter},0 ${width},${arrowHeadHeight}`}
      />
    </svg>
  );
}
...
function Coronation(props: {
  white: boolean;
  column: number;
  coroningWhite: boolean;
  squareSize: number;
  clearCoronation: () => void;
  coronate: (piece: PieceSymbol) => void;
}) {
  const {
    white,
    column,
  ...
  const pieces: PieceSymbol[] = [QUEEN, KNIGHT, ROOK, BISHOP];
  ...
  useEffect(() => {
    ...
    return () =>
      document.removeEventListener("mousedown", clickClearCoronation);
  }, []);
  ...
  return (
    <div
      ref={coronationRef}
      className="flex absolute bg-white z-[90] shadow-lg shadow-black/50 cursor-pointer"
      style={{
        flexDirection: top ? "column" : "column-reverse",
        top: top ? 0 : undefined,
        bottom: !top ? 0 : undefined,
        left: white ? column * squareSize : undefined,
        right: !white ? column * squareSize : undefined,
      }}
    >
      {pieces.map((piece, i) => (
        <div
      ...
      <div
        onClick={clearCoronation}
        className="bg-neutral-200 w-full flex justify-center items-center"
        style={{ height: squareSize / 2 }}
      >
        <Image
          draggable={false}
...
function Piece(props: {
  ...
  castleRookRef: RefObject<HTMLDivElement | null>;
  moved: boolean;
  isCastleRook: boolean;
  pieceColor: Color;
  pieceSymbol: PieceSymbol;
  drag: drag;
  setDrag: (dragging: drag) => void;
  id: string;
  boardRef: RefObject<HTMLDivElement | null>;
  setPlaying: (playing: boolean) => void;
  isCoronating: boolean;
}) {
  const {
    boardRef,
    pieceRef,
  ...
  function handlePieceDragStart(e: React.MouseEvent) {
    ...
    const startPosition = {
      x: elemenRect.x + elemenRect.width / 2,
      y: elemenRect.y + elemenRect.height / 2,
    };
  ...
  function handlePieceDrag(
    ...
    const movement = {
      x:
        Math.min(Math.max(e.clientX, limits.min.x), limits.max.x) -
        startPosition.x,
      y:
        Math.min(Math.max(e.clientY, limits.min.y), limits.max.y) -
        startPosition.y,
    };
  ...
  function handlePieceDragStop(
    ...
    if (
      ...
      Math.abs(movement.x) <= squareSize / 2 &&
      Math.abs(movement.y) <= squareSize / 2
    ) {
      newId = "";
    }
  ...
  return (
    <div
      ...
      style={{
        top: movement.y || "",
        left: movement.x || "",
        zIndex: drag.is && drag.id === id ? 100 : "",
      }}
    >
      <PieceSVG
        className="*:pointer-events-none"
        dataset={{ dontcleandrag: true }}
        piece={pieceSymbol}
        color={pieceColor}
        size={squareSize}
      />
...
export default function Board(props: {
  ...
  gameEnded: boolean;
  capture?: PieceSymbol;
  nextCapture?: PieceSymbol;
  castle?: "k" | "q";
  nextCastle?: "k" | "q";
  setAnimation: (animation: boolean) => void;
  result: result;
  arrows: arrow[];
  pushArrow: (arrow: arrow) => void;
  analyzeMove: (
    previousFen: string,
  ...
  setMaterialAdvantage: (materialAdvantage: number) => void;
  drag: drag;
  setDrag: (dragging: drag) => void;
  setPlaying: (playing: boolean) => void;
}) {
  const [hoverDrag, setHoverDrag] = useState("");
  const [coronation, setCoronation] = useState<coronation>({
    choosing: false,
    movement: [],
  });
  ...
  const castleRookFrom = getCastleRookFromSquare(
    forward ? castle : nextCastle,
    forward ? whiteMoving : !whiteMoving,
  );
  const castleRookTo = getCastleRookToSquare(
    forward ? castle : nextCastle,
    forward ? whiteMoving : !whiteMoving,
  );

  const castleRookMove: square[] =
    castleRookFrom && castleRookTo ? [castleRookFrom, castleRookTo] : [];

  const filteredHighlightStyle = filterHighlightStyle(HIGHLIGHT_COLORS);

  const highlightColor = highlightByRating
    ? filteredHighlightStyle[moveRating as keyof typeof filteredHighlightStyle]
        ?.color || boardThemes[boardTheme].highlight
    : boardThemes[boardTheme].highlight;
  const highlightRating =
    filteredHighlightStyle[moveRating as keyof typeof filteredHighlightStyle]
      ?.rating;
  ...
  function startArrow(x: number, y: number) {
    ...
    const square = {
      col: white ? colNumber : 7 - colNumber,
      row: white ? rowNumber : 7 - rowNumber,
    };
  ...
  function filterHighlightStyle(highlightStyle: typeof HIGHLIGHT_COLORS) {
    ...
    for (const key in highlightStyle) {
      ...
      if (usedRatings[rating])
        filteredHighlightStyle[rating as string] = {
          color: HIGHLIGHT_COLORS[rating],
          rating,
        };
    }
  ...
  async function handleMovePiece(e: React.MouseEvent, toSquare: string) {
    ...
    if (
      chess.get(from as Square)?.type === PAWN &&
      [0, 7].includes(formattedTo.row)
    ) {
  ...
  function animateMove(
    ...
    const distance = {
      ...
      y:
        (toElementPosition.bottom - fromElementPosition.bottom) *
        (white ? 1 : -1),
    };
  ...
  function getLegalMoves() {
    ...
    const moves = chess
      .moves({ square: drag.id as Square, verbose: true })
      .map((move) => move.to);
    return moves;
  }
  ...
  const boardColors = [
    boardThemes[boardTheme].white,
    boardThemes[boardTheme].black,
  ];

  const adaptedCoronationSquare = coronation.movement[0]
    ? adaptSquare(coronation.movement[0])
    : null;
  ...
  return (
    <div
      ...
      className="grid w-fit h-fit relative"
      style={{
        gridTemplateColumns: `repeat(8, ${squareSize}px)`,
        pointerEvents: drag.is ? "none" : "unset",
      }}
    >
      {(() => {
        ...
        for (const row of board) {
          ...
          for (const square of row) {
            ...
            if (rowNumber === (white ? 7 : 0)) {
              squareLetterGuide = (
                <span
                  style={{ right: rightSize, color: guideColor }}
            ...
            if (columnNumber === (white ? 0 : 7)) {
              squareNumGuide = (
                <span
                  style={{ left: leftSize, color: guideColor }}
            ...
            const iconTranslateX = (
              white ? columnNumber === 7 : columnNumber === 0
            )
              ? -5
              : 35;
            const iconTranslateY = (white ? rowNumber === 0 : rowNumber === 7)
              ? 5
              : -35;
            ...
            move.forEach((square, i) => {
              ...
              if (
                highlightedSquare.col === columnNumber &&
                highlightedSquare.row === rowNumber
              ) {
                highlighted = (
                  <div
                    style={{ backgroundColor: highlightColor }}
                    className={`absolute z-[10] top-0 left-0 w-full h-full opacity-50 ${rounded}`}
                  />
                );
                if (i === 1)
                  highlightedIcon =
                    highlightRating && i === 1 ? (
                      <RatingSVG
                        className="absolute top-0 right-0 z-[80] pointer-events-none"
                        style={{
            ...
            if (
              (squareId === drag.id && !highlighted) ||
              (coronation.choosing &&
                adaptedCoronationSquare?.col === columnNumber &&
                adaptedCoronationSquare?.row === rowNumber)
            ) {
              highlighted = (
                <div
                  style={{ backgroundColor: boardThemes[boardTheme].highlight }}
                  className={`absolute top-0 left-0 w-full h-full opacity-50 ${rounded}`}
                />
              );
            }
                    ...
                    if (result === "1-0")
                      ...
                      resultIcon = (
                        <ResultSVG
                          className="absolute top-0 right-0 z-[60]"
                          style={{
                            transform: `translateX(${iconTranslateX}%) translateY(${iconTranslateY}%) rotate(-90deg)`,
                          }}
                          size={iconSize}
                          result="defeat"
                        />
            ...
            const adaptedToAnimateSquare = toAnimateSquare
              ? adaptSquare(toAnimateSquare)
              : { col: NaN, row: NaN };
            const moved =
              adaptedToAnimateSquare.col === columnNumber &&
              adaptedToAnimateSquare.row === rowNumber;

            const isCastleRook = forward
              ? castleRookTo?.col === columnNumber &&
                castleRookTo?.row === rowNumber
              : castleRookFrom?.col === columnNumber &&
                castleRookFrom?.row === rowNumber;
            ...
            const hoverDragSquare = (
              <div
                style={{
                  display: drag.is ? "" : "none",
                  opacity: hoverDrag === squareId ? "100" : "",
                  borderWidth: squareSize * 0.05,
                }}
                onMouseEnter={() => setHoverDrag(squareId)}
                onMouseLeave={() => setHoverDrag("")}
                className={`absolute top-0 z-[30] left-0 w-full h-full border-opacity-65 opacity-0 block border-white pointer-events-auto ${rounded}`}
              />
            );
            ...
            const legalMove = legalMoves.includes(squareId as Square) ? (
              piece ? (
                <div
                  onMouseEnter={() => setHoverDrag(squareId)}
                  onMouseLeave={() => setHoverDrag("")}
                  onMouseDown={(e) => handleMovePiece(e, squareId)}
                  onMouseUp={(e) => handleMovePiece(e, squareId)}
                  className="absolute w-full h-full z-[40] top-0 left-0 cursor-grab pointer-events-auto"
                >
                  <div
                    style={{ borderWidth: squareSize * 0.12 }}
                    className="border-black opacity-[15%] w-full h-full rounded-full"
                  />
                </div>
              ) : (
                <div
                  ...
                  onMouseDown={(e) => handleMovePiece(e, squareId)}
                  onMouseUp={(e) => handleMovePiece(e, squareId)}
                  style={{ opacity: showLegalMoves ? "" : 0 }}
                  className="absolute w-full h-full z-[40] top-0 left-0 flex justify-center items-center pointer-events-auto"
                >
                  <div className="bg-black opacity-[15%] w-[30%] h-[30%] rounded-full" />
                </div>
              )
            ) : null;
            ...
            squares.push(
              <div
                ...
                className={`font-bold relative ${rounded}`}
              >
                {squareNumGuide}
                {squareLetterGuide}
                {piece}
                {highlighted}
                {resultIcon ? null : highlightedIcon}
                {resultIcon}
                {hoverDragSquare}
                {legalMove}
              </div>,
            ...
            if (square) {
              if (square?.color === WHITE) {
                newMaterialAdvantage +=
                  PIECES_VALUES[square.type as keyof typeof PIECES_VALUES];
              } else {
                newMaterialAdvantage -=
                  PIECES_VALUES[square.type as keyof typeof PIECES_VALUES];
              }
      ...
      {(() => {
        ...
        if (
          (highlightRating === "book" && arrowAfterMove) ||
        ...
        return adaptedMove ? (
          <Arrow
            move={adaptedMove}
            squareSize={squareSize}
            class="fill-bestArrow stroke-bestArrow"
            white={white}
          />
        ) : (
          ""
      ...
      {arrows.map((move, i) => {
        const singleSquare =
          JSON.stringify(move[0]) === JSON.stringify(move[1]);

        return (
          <Arrow
            key={i}
            move={singleSquare ? [move[0]] : move}
            squareSize={squareSize}
            class={
              singleSquare
                ? "bg-badArrow"
                : "fill-normalArrow stroke-normalArrow"
            }
            white={white}
          />
        );
