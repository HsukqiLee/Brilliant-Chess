      ...
      >
        ...
        <div
          style={{
            ...
            display: showArrows ? "" : "none",
          }}
          className="w-3 h-3 rounded-full absolute right-3"
        />
      </button>
      <button
        onClick={toggleArrowAfterMove}
        type="button"
        className="flex flex-row gap-2 items-center hover:text-foregroundHighlighted hover:bg-black transition-colors w-full relative p-2"
      >
        <div className="grid grid-cols-2 w-fit relative">
          {Array.from({ length: 4 }).map((_, i) => {
            ...
            return (
              <div
                ...
                className="h-5 w-5 relative"
              >
                {(i === 2 || i === 3) && arrowAfterMove ? (
                  <div
                    className="w-full h-full absolute top-0 left-0 opacity-50"
                    style={{ backgroundColor: "var(--highlightBoard)" }}
                  />
                ) : null}
                {(i === 2 && !arrowAfterMove) || (i === 3 && arrowAfterMove) ? (
                  <PieceSVG
                    className="absolute z-10 top-0 left-0"
                    piece={QUEEN}
                    size={20}
