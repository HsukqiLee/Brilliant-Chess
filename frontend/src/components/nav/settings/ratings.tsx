...
function serializeRatings(usedRatings: usedRatings) {
  ...
  for (const rating of ratings) {
    if (usedRatings[rating as keyof typeof usedRatings])
      serializedRatings.push(1);
...
function unserializeRatings(serializedUsedRatings: string) {
  ...
  if (serializedRatings.length !== ratings.length)
    throw new Error("Invalid serialized ratings");

  for (const [i, rating] of ratings.entries()) {
    if (isNaN(serializedRatings[i]))
      throw new Error("Invalid serialized ratings");

    if (serializedRatings[i])
      unserializedRatings[rating as keyof typeof unserializedRatings] = true;
    else
      unserializedRatings[rating as keyof typeof unserializedRatings] = false;
  }
...
export default function Ratings() {
  ...
  const [highlightByRating, setHighlightByRating] =
    configContext.highlightByRating;
  ...
  function toggleHighlightByRating() {
    ...
    localStorage.setItem(
      "highlightByRating",
      String(Number(newHighlightByRating)),
    );
  ...
  return (
    <section>
      <h1 className="block bg-backgroundBoxBox font-bold text-nowrap p-3 text-foreground">
        Ratings
      </h1>
      <button
        onClick={toggleHighlightByRating}
        type="button"
        className="flex flex-row gap-2 items-center hover:text-foregroundHighlighted hover:bg-black transition-colors w-full relative p-2"
      >
              ...
              >
                {i === 2 || i === 3 ? (
                  <div
                    ...
                    style={{
                      backgroundColor: highlightByRating
                        ? "var(--highlightGreat)"
                ...
                {i === 3 ? (
                  <>
                    ...
                    <RatingSVG
                      rating="great"
                      size={12}
                      className="absolute top-0 right-0 translate-x-1/2 translate-y-[-50%]"
                    />
                  </>
                ) : null}
              </div>
        ...
        <div
          style={{
            ...
            display: highlightByRating ? "" : "none",
          }}
      ...
      {ratings.map((rating, i) => {
        const color =
          rating === "forced"
            ? "var(--highlightGood)"
            : `var(--highlight${capitalizeFirst(rating)})`;
          ...
          >
            ...
            <div
              style={{
                ...
                display: usedRatings[rating as keyof typeof usedRatings]
                  ? ""
                  : "none",
              }}
              className="w-3 h-3 rounded-full absolute right-3"
