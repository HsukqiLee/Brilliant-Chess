...
export const DONATE_URL =
  "https://www.paypal.com/donate/?hosted_button_id=S8SWJBNYZ2WFW";
...
export default function Nav() {
  ...
  const botLinks: BotLinks[] = [
    ...
    },
    {
      ...
      href: "/attributions",
      icon: (props: { className: string }) => (
        <Licenses class={props.className} />
      ),
    },
  ];
    ...
    <nav className="flex flex-col navTop:h-screen navTop:w-max w-screen relative">
      <div className="navTop:pt-1 navTop:pb-6 navTop:h-full w-full overflow-y-auto bg-backgroundBox flex navTop:flex-col flex-row justify-between select-none navTop:items-start items-stretch">
        <div ref={topLinksRef} className="flex navTop:flex-col flex-row">
          <Link
            draggable={false}
            onMouseEnter={() => setOpenedMenu(null)}
            href="/"
            className="flex flex-row gap-1 font-extrabold text-xl navTop:p-3 p-1.5 transition-colors hover:bg-backgroundBoxHover hover:text-foregroundHighlighted"
          >
            <Image
              draggable={false}
              height={30}
            ...
            <div className="h-fit w-fit reduceNav:block hidden">
              Brilliant<span className="text-sm font-light">Chess</span>
            </div>
          </Link>
          {topLinks.map((link, i) => {
            return (
              <button
                onClick={link?.click}
                onMouseEnter={link?.hover}
                onMouseLeave={link?.unHover}
                type="button"
                key={i}
                className="text-lg outline-none font-bold navTop:px-3 navTop:py-2 p-1.5 hover:bg-backgroundBoxHover hover:text-foregroundHighlighted transition-colors flex flex-row gap-2"
              >
                <Image
                  draggable={false}
                  height={30}
          ...
          {botLinks.map((link, i) => {
            return (
              <Link
                ...
                target="_blank"
                href={link.href}
                className="flex flex-row gap-2 h-full navTop:h-fit reduceNav:px-3 reduceNav:justify-start justify-center items-center navTop:py-2 p-2 group hover:bg-backgroundBoxHover text-foregroundGrey hover:text-foregroundHighlighted transition-colors"
              >
      ...
      <div
        ...
        className="navTop:h-full h-fit max-h-[calc(100vh-42px)] navTop:max-h-full z-[500] p-2 bg-backgroundBoxDarker absolute navTop:left-full top-full navTop:top-0 select-none navTop:w-fit w-fit min-w-[300px] overflow-y-auto"
      >
